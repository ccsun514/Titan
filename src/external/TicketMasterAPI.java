...
public class TicketMasterAPI implements ExternalAPI{
	...
	//search path provided by ticketMaster
	...
	public List<Item> search(double lat, double lon, String term) {
		...
		//urlEncode(url cannot no space or some special characters)
	 ...
	 * Helper methods
	 */
	// Convert JSONArray(an array of JSONObject event) to a list of item objects.
	private List<Item> getItemList(JSONArray events) throws JSONException {
		...
		for (int i = 0; i < events.length(); i++) {
			...
			//check the FieldOrNull(avoid NullPointerException) check return == null ? null : getValue()
